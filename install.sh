...
termux_notice() {
cat <<'EOF'
HermesLaunch v1.1.0 ditujukan untuk VPS Linux + systemd.
...
check_platform() {
  [[ "$(uname -s)" == "Linux" ]] || die "HermesLaunch v1.1.0 hanya mendukung Linux VPS."
  command -v systemctl >/dev/null 2>&1 || die "systemd tidak ditemukan. HermesLaunch v1.1.0 membutuhkan systemd."
  if is_termux; then
    termux_notice
    exit 2
  fi
